...
from TADV.tadp_svd import TADPVid
import datetime
...
class HMDB51DataModule(pl.LightningDataModule):
    def __init__(self, video_path, split_file_path, num_frames, max_frames, step, format, batch_size, num_workers, normalize_videos):
        ...
        norm_fn = T.Normalize(mean=(128,), std=(128,))
        def normalize(video):
...
def main():
    ...
    dataset_len = len(datamodule.train_dataloader())*batch_size
    model = TADPVid(class_names=datamodule.train.classes, text_conditioning=args.text_conditioning,
                    blip_caption_path=blip_caption_path, freeze_backbone=args.freeze_backbone, 
                    log_ca=args.log_ca, max_epochs=max_epochs, dataset_len=dataset_len)
